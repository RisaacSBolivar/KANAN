...
#Creando una variable de tipo lidar
lidar = dl.Lidar()

lidar.build_windows()
current =lidar.windows.upper_center.min
print(current)
...
while True:
    lidar.build_windows()
    current =lidar.windows.upper_center.min
    distance = lidar.windows.upper_center.mean
    print(current)
    if (current < 10):
        client.hoverAsync().join()
        while current < 10 :
            altura-=2
            client.moveByVelocityZAsync(0,0,altura,1, airsim.DrivetrainType.MaxDegreeOfFreedom, airsim.YawMode(False, 0)).join()
            lidar.build_windows()
            current =lidar.windows.upper_center.min
    ...
    if ( current > pos):
        client.moveByVelocityZAsync(2,2,altura,1, airsim.DrivetrainType.ForwardOnly, airsim.YawMode(False, 0)).join()
        """
        ...
        change = 0
        min = (lidar.windows.upper_left.min,lidar.windows.upper_center.min,
               lidar.windows.upper_right.min)
        driving = min
