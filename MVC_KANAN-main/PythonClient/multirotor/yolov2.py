...
def deteccionUnica(img_,height, width, channels):
    ...
    path1 = os.path.abspath(__file__)[:-10] + '/yolov3.weights'
    path2 = os.path.abspath(__file__)[:-10] + '/yolov3.cfg'
    net = cv2.dnn.readNet(path1, path2)
...
def yolo(opcion,tiempoLimite,img):
    if opcion==1:
        return deteccionContinua(tiempoLimite)
    if opcion==2:
        return deteccionUnica(img)


def main():
    tiempoLimite=10.0
    img = cv2.imread("photo_1.png")
    print(yolo(2,tiempoLimite,img))
